...
void LUND_fromGENIE_C() {

  TFile* inFile = new TFile("/w/hallb-scshelf2102/clas/claseg2/apapadop/apapadop_C_6GeV_GEM21_11b_Q2_0_6_Rad.root");
  TString lundPath = "../lundfiles/";
	...
	int targA = 2;
	int targZ = 1;
	double RES_ID; //WAS targP = 0.; // polarization
	...
	for (int iFiles = 1; iFiles < nFiles; iFiles++)
	  {
	    ...
	    outstring = Form(formatstring, nf_mod, targA, targZ, RES_ID/*targP*/, beamP, beamType, beamE, interactN, i, code);
	    
	    outfile << outstring; 

	    double upstream_foil = -4.875; //cm
